...
import { CommandProps } from './SuggestView'
...
export const defaultItems = [
  {
    ...
    command: ({ view, range }: CommandProps) => {
      // const tr = editor.view.state.schema.nodes.bulletListItem.create()
      const text = view.state.schema.text('test')
      const p = view.state.schema.nodes.paragraph.create(null, text)
      const tr = view.state.tr.insert(
        range.from,
        view.state.schema.nodes.bulletListItem.create(null, p)
      )
      view.dispatch(tr)
...
export const suggestSlashCommands = ({ view, query }: { view: EditorView; query: string }) => {
  const selection = view.state.selection
  const isInTable = ProsemirrorNodes.hasAncestorNode(view, () => selection.$from.pos, 'table')
