...
export const frontMatter = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return ['front-matter', mergeAttributes(this.options.HTMLAttributes, HTMLAttributes), 0]
  },
        ...
        handler(props): void {
          ...
          if (from === 1) {
            tr.replaceRangeWith(
              from,
              to,
              type.create({ content: schema.text(match[0], []), attrs: { class: 'front-matter' } })
            ).insertText(match[0])
          }
        }
      })
          ...
          setup(markdownit) {
            const frontMatterPlugin = (md, cb) => {
              ...
              function frontMatter(state, startLine, endLine, silent) {
                ...
                // state.push('front_matter_begin', 'front-matter', 1)
                // state.push('text', token.meta, 0)
                // state.push('front_matter_end', 'front-matter', -1)
                cb(token.meta)
              ...
              md.renderer.rules.front_matter = (tokens, idx) => {
                ...
                const html = `<front-matter class="front-matter">${meta}</front-matter>`
                return html
              }
            }
            markdownit.use(frontMatterPlugin, (fm) => {})
          }
        }
      }
    }
